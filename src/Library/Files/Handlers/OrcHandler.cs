...
// Se usa Creator ya que se crea una instancia de Orc.
...
{
    public class OrcHandler : TypeHandler
    {
        public OrcHandler(TypeHandler nextHandler) : base(nextHandler)
        {
        }
        ...
        {   
            ...
            Orc orc = new Orc(values[0],Convert.ToInt32(values[1]),Convert.ToInt32(values[2]),Convert.ToInt32(values[3]));

            handlerRequest.ListOfCharacter.Add(orc);
